<?php
	...
	class Painel
	{
		...
		public static function generateSlug($str){ //Vou passar como parâmetro o $str (é um nome normal, não está formatado)
			//No preg_replace passamos extensões Regulares
			//Dentro da /()/ é onde vai toda a nossa expressão
			//Estou dizendo para pegar tudo que está aqui dentro /( )/ e substitua pela letra ' '
			$str = mb_strtolower($str); //Vai converter tudo para letras minusculas
		...
		public static function carregarPagina(){
			if(isset($_GET['url'])){
				...
				if(file_exists('pages/'.$url[0].'.php')){
					...
					/*Sistema de Rotas
					if(Router::get('vizualizar-empreendimento/?',function($par){
						include('views/vizualizar-empreendimento.php');
					})){
					}else if(Router::post('vizualizar-empreendimento/?',function($par){
						include('views/vizualizar-empreendimento.php');
					})){
					}else{
					Página não existe!
		...
		public static function alert($tipo,$mensagem){
			if($tipo == 'sucesso'){
				echo '<div class="box-alert sucesso" id="boxSucess"><i data-feather="check-circle"></i> '.$mensagem.'</div>';
			}else if($tipo == 'erro'){
				echo '<div class="box-alert erro" id="boxError"><i data-feather="x-circle"></i> '.$mensagem.'</div>';
			}else if($tipo == 'atencao'){
				echo '<div class="box-alert sucesso atencao" id="boxAttention"><i data-feather="alert-circle"></i> '.$mensagem.'</div>';
			}
		}
